...
resource "ibm_dns_record" "bastion" {
  ...
  host               = "${var.cluster_id}${var.name_prefix}-bastion-${count.index}.${var.cluster_id}"
...
resource "ibm_dns_record" "bootstrap" {
  ...
  domain_id          = data.ibm_dns_domain.domain.id
  host               = "bootstrap${var.name_prefix}.${var.cluster_id}"
  responsible_person = "root.${var.cluster_domain}."
  ttl                = 900
  type               = "a"
}
resource "ibm_dns_record" "master" {
  ...
  domain_id          = data.ibm_dns_domain.domain.id
  host               = "master-${count.index}${var.name_prefix}.${var.cluster_id}"
  responsible_person = "root.${var.cluster_domain}."
  ttl                = 900
  type               = "a"
}
resource "ibm_dns_record" "worker" {
  ...
  domain_id          = data.ibm_dns_domain.domain.id
  host               = "worker-${count.index}${var.name_prefix}.${var.cluster_id}"
  responsible_person = "root.${var.cluster_domain}."
  ttl                = 900
  type               = "a"
}
